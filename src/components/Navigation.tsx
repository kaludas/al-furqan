  ...
  return (
    <>
      <nav className={cn(
        ...
        isScrolled ? "glass-card py-3 border-b border-glass" : "py-6 bg-transparent"
      )}>
        <div className="container max-w-7xl flex items-center justify-between">
          <button onClick={() => scrollTo("hero")} className="flex items-center gap-2">
            <span className="text-2xl font-display text-gradient-gold">القرآن</span>
            <span className="text-foreground font-display text-lg">Al-Furqan</span>
          </button>
        ...
        <div className="relative h-full flex flex-col items-center justify-center gap-4 p-8">
          {navItems.map(item => (
            <button
              ...
              onClick={() => scrollTo(item.id)}
              className={cn(
                "flex items-center gap-4 px-6 py-3 rounded-xl text-lg font-display transition-all duration-300",
                activeSection === item.id
                  ? "bg-primary/20 text-primary"
